...
If you want a single Node.js service to host both the API and the frontend, keep the built frontend at `frontend/dist`. The backend will serve that build automatically when it exists, so routes like `/login` and `/organizations` open directly from the same domain.
